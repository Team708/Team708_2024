// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands;

import edu.wpi.first.wpilibj2.command.ParallelRaceGroup;

//Command imports
import frc.robot.commands.shooter.SetShooterSpeedSpeaker;
import frc.robot.commands.Feeder.FeederAutomatic;
import frc.robot.commands.intake.IntakeAllIn;
//Subsystem imports
import frc.robot.subsystems.Intake;
import frc.robot.subsystems.Feeder;
import frc.robot.subsystems.Shooter;
// NOTE:  Consider using this command inline, rather than writing a subclass.  For more
// information, see:
// https://docs.wpilib.org/en/stable/docs/software/commandbased/convenience-features.html
public class AllSystemsOn extends ParallelRaceGroup {
  /** Creates a new AllSystemsOn. */
  public AllSystemsOn(Intake m_intake, Feeder m_feeder, Shooter m_shooter) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
      new IntakeAllIn(m_intake),
      new FeederAutomatic(m_feeder),
      new SetShooterSpeedSpeaker(m_shooter)
    );
  }
}
